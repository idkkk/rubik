...
export default class Search extends Component {
	...
	setInputValue(refs,json) {
		...
		let descriptionInput = ReactDOM.findDOMNode(refs.description).firstChild
  	...
  	getInputValue(refs) {
    	return {name: ReactDOM.findDOMNode(refs.name).firstChild.value,
    		description: ReactDOM.findDOMNode(refs.description).firstChild.value,
	...
	handleClearClick() {
    	...
    	this.setInputValue(this.refs,{name:"",description:""})
  	}
			    ...
			    <Row gutter={16}>
			      ...
			      <Col sm={8}>
			        <FormItem label="描述" labelCol={{ span: 10 }} wrapperCol={{ span: 14 }}>
			          <Input ref="description" placeholder="请输入搜索描述" size="default" />
			        </FormItem>
			      </Col>
			    </Row>
			    <Row>
